...
static void mv(char *name, char *newtype)
{
  ...
  snprintf(old, sizeof(old), "%s%c%s", POOLDIR, DIRSEP, name);
#ifdef SHORTNAMES
  ...
  name[0] = newtype[0];
#endif /* else if not SHORTNAMES */
  snprintf(new, sizeof(new), "%s%c%s", POOLDIR, DIRSEP, name);
  rename(old, new);
}
...
int latent_read(void)
{
  ...
  if (d != NULL)
	...
	break;
      if (is(e->d_name, "lat")) {
	snprintf(path, sizeof(path), "%s%c%s", POOLDIR, DIRSEP, e->d_name);
	f = fopen(path, "rb");
	if (f != NULL) {
	  fscanf(f, "%*d %ld\n", &then);
	  fclose(f);
 ...
 */
{
  ...
  if (d != NULL)
	...
	break;
      if (e->d_name[0] != '.') {
	snprintf(path, sizeof(path), "%s%c%s", dir, DIRSEP, e->d_name);
	path[PATHMAX-1]='\0';
	f = fopen(path, "rb");
	if (f != NULL) {
	  buf_clear(msg);
 ...
 */
{
  ...
  snprintf(path, sizeof(path), "%s%c%s", normalized, DIRSEP, "new");
  path[PATHMAX-1]='\0';
  mailin_maildir_one(path);
  snprintf(path, sizeof(path), "%s%c%s", normalized, DIRSEP, "cur");
  path[PATHMAX-1]='\0';
  mailin_maildir_one(path);
  return (0);
}
...
#ifdef SHORTNAMES
  snprintf(tmpname, PATHMAX, "%s%c%02x%02x%02x%02x.tmp", POOLDIR, DIRSEP, rnd_byte(), rnd_byte(),
	  rnd_byte(), rnd_byte());
  strcpy(path, tmpname);
  memcpy(path + strlen(path) - 3, type, 3);
#else /* end of SHORTNAMES */
  snprintf(tmpname, PATHMAX, "%s%ct%02x%02x%02x%02x%02x%02x%01x", POOLDIR, DIRSEP, rnd_byte(),
