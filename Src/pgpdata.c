...
int pgp_rsa(BUFFER *in, BUFFER *k, int mode)
{
  ...
  {
    BIGNUM *n, *e, *d = NULL, *p = NULL, *q = NULL, *iqmp = NULL;
    BIGNUM *dmp1 = NULL, *dmq1 = NULL;

    mpi_get(k, mpi);
    n = BN_bin2bn(mpi->data, mpi->length, NULL);

    if (mpi_get(k, mpi) < 0)
      goto end;
    e = BN_bin2bn(mpi->data, mpi->length, NULL);

    if (mode == PK_DECRYPT || mode == PK_SIGN) {
      if (mpi_get(k, mpi) < 0)
	goto end;
      d = BN_bin2bn(mpi->data, mpi->length, NULL);

#if 1
      /* compute auxiluary parameters */
      mpi_get(k, mpi);		/* PGP'p is SSLeay's q */
      q = BN_bin2bn(mpi->data, mpi->length, NULL);

      mpi_get(k, mpi);
      p = BN_bin2bn(mpi->data, mpi->length, NULL);

      if (mpi_get(k, mpi) < 0)
	goto end;
      iqmp = BN_bin2bn(mpi->data, mpi->length, NULL);
     ...
     /* remail==2: encrypt the secring */
{
...

  {
    ...
    BN_set_word(e, 17);
    k = RSA_new();
    if (k == NULL || RSA_generate_key_ex(k, bits == 0 ? 1024 : bits, e, NULL) != 1) {
      BN_free(e);
      RSA_free(k);
      k = NULL;
      err = -1;
  ...
  {
    const BIGNUM *kn, *ke, *kd, *kp, *kq, *kiqmp;

    buf_appendc(skey, PGP_ES_RSA);
    RSA_get0_key(k, &kn, &ke, &kd);
    RSA_get0_factors(k, &kp, &kq);
    RSA_get0_crt_params(k, NULL, NULL, &kiqmp);
    mpi_bnput(skey, (BIGNUM *)kn);
    mpi_bnput(skey, (BIGNUM *)ke);
  ...
  if (pass != NULL && pass->length > 0 && remail != 2) {
    ...
    mpi_bnputenc(skey, (BIGNUM *)kd, skalgo, dk, iv);
    mpi_bnputenc(skey, (BIGNUM *)kq, skalgo, dk, iv);
    mpi_bnputenc(skey, (BIGNUM *)kp, skalgo, dk, iv);
    mpi_bnputenc(skey, (BIGNUM *)kiqmp, skalgo, dk, iv);

    buf_clear(p);
    mpi_bnput(p, (BIGNUM *)kd);
    mpi_bnput(p, (BIGNUM *)kq);
    mpi_bnput(p, (BIGNUM *)kp);
    mpi_bnput(p, (BIGNUM *)kiqmp);
  }
...
static void *params(int dsa, int bits)
{
  ...
  if (f != NULL) {
	...
	if (bits == m) {
	  ...
	  while (fgets(line, sizeof(line), f) != NULL) {
		...
		l = buf_geti(p);
		buf_get(p, n, l);
		bp = BN_bin2bn(n->data, n->length, NULL);
		l = buf_geti(p);
		buf_get(p, n, l);
		bq = BN_bin2bn(n->data, n->length, NULL);
		l = buf_geti(p);
		buf_get(p, n, l);
		bg = BN_bin2bn(n->data, n->length, NULL);
		DSA_set0_pqg(k, bp, bq, bg);
	      } else {
		BIGNUM *bp, *bg;
  ...
  if (dsa) {
    if (k == NULL) {
      ...
      p = buf_new();
      {
	const BIGNUM *kp, *kq, *kg;

	DSA_get0_pqg(k, &kp, &kq, &kg);
	l = BN_bn2bin(kp, b);
	buf_appendi(p, l);
	buf_append(p, b, l);
	l = BN_bn2bin(kq, b);
     ...
     /* remail==2: encrypt the secring */
{
  ...
  buf_appendl(skey, now);
  {
    const BIGNUM *sp, *sq, *sg, *spub, *spriv;

    buf_appendc(skey, PGP_S_DSA);
    DSA_get0_pqg(s, &sp, &sq, &sg);
    DSA_get0_key(s, &spub, &spriv);
    mpi_bnput(skey, (BIGNUM *)sp);
    mpi_bnput(skey, (BIGNUM *)sq);
    mpi_bnput(skey, (BIGNUM *)sg);
    mpi_bnput(skey, (BIGNUM *)spub);

    mpi_bnput(secret, (BIGNUM *)spriv);
  }
  ...
  {
    const BIGNUM *ep, *eg, *epub, *epriv;

    buf_appendc(subkey, PGP_E_ELG);
    DH_get0_pqg(e, &ep, NULL, &eg);
    DH_get0_key(e, &epub, &epriv);
    mpi_bnput(subkey, (BIGNUM *)ep);
    mpi_bnput(subkey, (BIGNUM *)eg);
    mpi_bnput(subkey, (BIGNUM *)epub);

    buf_clear(secret);
    mpi_bnput(secret, (BIGNUM *)epriv);
  }
  buf_appendi(secret, pgp_csum(secret, 0));
  makeski(secret, pass, remail);
...
int pgp_dsasign(BUFFER *data, BUFFER *key, BUFFER *out)
{
  ...
  mpi = buf_new();
  {
    BIGNUM *dp, *dq, *dg, *dpub, *dpriv;

    mpi_get(key, mpi);
    dp = BN_bin2bn(mpi->data, mpi->length, NULL);
    mpi_get(key, mpi);
    dq = BN_bin2bn(mpi->data, mpi->length, NULL);
    mpi_get(key, mpi);
    dg = BN_bin2bn(mpi->data, mpi->length, NULL);
    mpi_get(key, mpi);
    dpub = BN_bin2bn(mpi->data, mpi->length, NULL);
    if (mpi_get(key, mpi) == -1) {
      BN_free(dp); BN_free(dq); BN_free(dg); BN_free(dpub);
      goto end;
    }
    dpriv = BN_bin2bn(mpi->data, mpi->length, NULL);
    if (!DSA_set0_pqg(d, dp, dq, dg) || !DSA_set0_key(d, dpub, dpriv)) {
      BN_free(dp); BN_free(dq); BN_free(dg); BN_free(dpub); BN_free(dpriv);
      goto end;
    }
  }
  ...
  if (sig) {
    ...
    DSA_SIG_get0(sig, &sr, &ss);
    buf_prepare(b, BN_num_bytes(sr));
    b->length = BN_bn2bin(sr, b->data);
    mpi_put(out, b);
    b->length = BN_bn2bin(ss, b->data);
    mpi_put(out, b);
  }
 end:
  buf_free(mpi);
